...
/** In order to have operations with constant time complexity - O(1) their implementation need to use only Q(1) downstream operations.
  * Of the existing collections [[List]] has head, tail, prepend and isEmpty complexity as Q(1) [[http://docs.scala-lang.org/overviews/collections/performance-characteristics.html]].
...
class FifoQueue[+T](private val in: List[T] = Nil, private val out: List[T] = Nil) extends Queue[T] {
  ...
  override def remove: (Option[T], Queue[T]) = out match {
    case Nil =>
      if (in.nonEmpty) {
        val inRev = in.reverse
        Some(inRev.head) -> new FifoQueue(Nil, inRev.tail)
      }
      else (None, this)
